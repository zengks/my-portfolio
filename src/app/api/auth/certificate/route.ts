...
import { getUserCertificate, updateUserCertificate } from '@/controllers/userCertificateController';
...
export async function GET(): Promise<NextResponse> {
	return withAuth(async (session) => {
		try {
			...
			return NextResponse.json({ Error: 'Failed to fetch user certificate.' }, { status: 500 });
		}
	})();
}
...
export async function PUT(request: NextRequest): Promise<NextResponse> {
	return withAuth(async () => {
		try {
			...
			return NextResponse.json({ Error: 'Failed to update user certificate' }, { status: 500 });
		}
	})(request);
}
